...
export const addStudentAPI = async (data) => {
  try {
    const result = await axios.post(`http://backend:5000/api/classroom/student`, data);
...
export const addMultiStudentAPI = async (data) => {
  try {
    const result = await axios.post(`http://backend:5000/api/classroom/student/multidata`, { data });
    console.log(result);
    if (result.status == 200) {
