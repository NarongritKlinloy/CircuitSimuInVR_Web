...
export function SignIn() {
  ...
  const handleGoogleLoginSuccess = (credentialResponse) => {
    try {
      ...
      // กำหนดสิทธิ์ Developer team
      if (
        ["65015041@kmitl.ac.th", "65015123@kmitl.ac.th", "65015101@kmitl.ac.th", "65015168@kmitl.ac.th", "boomza53214@gmail.com"].includes(email)
      ) {
        Swal.fire({
          icon: "success",
          ...
          if (result.isConfirmed) {
            ...
            signInAPI(email, name, role === "admin" ? 2 : 1, date);
            addLogAPI(email, 0, 0);
            navigate(role === "admin" ? "/dashboard/home" : "/teacher/home");
          }
        });
        return;
      }

      // ตรวจสอบว่าเป็นนักเรียน (รหัส 8 ตัวเลข)
      if (/^\d{8}@kmitl\.ac\.th$/.test(email)) {
        Swal.fire({
          icon: "error",
          title: "Access Denied",
  ...
  return (
    <GoogleOAuthProvider clientId="536241701089-qa3r463eobp3nt5754klif634rcr9f42.apps.googleusercontent.com">
      <GoogleLogin
        ...
        useOneTap
        redirect_uri="https://smith11.ce.kmitl.ac.th/auth/sign-in"  // ✅ เปลี่ยนให้ตรงกับ Google Console
      />
        ...
        <div className="w-full lg:w-1/2 p-8">
          ...
          <div className="bg-white border border-gray-300 shadow-lg rounded-lg overflow-hidden">
            ...
            <div className="p-6 text-center">
              ...
              <Typography variant="paragraph" color="blue-gray" className="text-lg font-normal mt-2">
