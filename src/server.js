...
app.post("/register", async (req, res) => {
  ...
  try {
    ...
    if (existingUser.length > 0) {
      await db.query(
        "UPDATE user SET last_active = CONVERT_TZ(NOW(), 'UTC', 'Asia/Bangkok'), role_id = ? WHERE uid = ?",
        [role_id, email]
      );
      ...
      await db.query(
        "INSERT INTO user (uid, name, role_id, last_active) VALUES (?, ?, ?, CONVERT_TZ(NOW(), 'UTC', 'Asia/Bangkok'))",
        [email, name, role_id]
      );
...
app.put("/api/simulator/update", async (req, res) => {
  try {
    ...
    const sql = `
      UPDATE savecircuit 
      SET circuit_json = ?, circuit_date = CONVERT_TZ(NOW(), 'UTC', 'Asia/Bangkok') 
...
app.post("/api/log/visitunity", async (req, res) => {
  try {
    ...
    const sql = `INSERT INTO log (uid, log_time, log_type, practice_id) VALUES (?, CONVERT_TZ(NOW(), 'UTC', 'Asia/Bangkok'), ?, ?)`;
...
app.post("/api/classroom/student", async (req, res) => {
  ...
  try {
    ...
    const sql_enroll = "INSERT INTO enrollment (uid, class_id, enroll_date) VALUES (?, ?, CONVERT_TZ(NOW(), 'UTC', 'Asia/Bangkok'))";
...
app.post("/api/classroom/student/multidata", async (req, res) => {
  ...
  try {
    ...
    const sql_insert_user = "INSERT INTO user (uid, name, role_id, last_active) VALUES(?, ?, 3, CONVERT_TZ(NOW(), 'UTC', 'Asia/Bangkok'))";
    const sql_enroll_select = "SELECT * FROM enrollment WHERE uid = ?";
    const sql_enroll = "INSERT INTO enrollment (uid, class_id, enroll_date) VALUES (?, ?, CONVERT_TZ(NOW(), 'UTC', 'Asia/Bangkok'))";
...
app.put("/api/classroom/sec/:uid", async (req, res) => {
  try {
    ...
    // now.setHours(now.getHours() + 7); // เพิ่ม 7 ชั่วโมงให้ตรงกับเวลาประเทศไทย
    // const enrollDate = now.toISOString().slice(0, 19).replace("T", " ");
    const sql_enroll = ("UPDATE enrollment SET class_id = ?, enroll_date = CONVERT_TZ(NOW(), 'UTC', 'Asia/Bangkok') WHERE uid = ?");
...
app.put("/api/update-notification", async (req, res) => {
  ...
  const sql = `
    UPDATE report
    SET report_isread = 1, report_dateread = CONVERT_TZ(NOW(), 'UTC', 'Asia/Bangkok') 
    WHERE report_id = ?
  `;
